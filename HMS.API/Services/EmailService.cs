    ...
    public class EmailService : IEmailService
    {
            ...
            decimal cancellationFee)
        {
            ...
            message.HtmlBody = BuildCancellationHtml(
                guestName, referenceNumber, hotelName, checkIn, checkOut, cancellationFee);
            ...
            string hotelName, DateTime checkOut)
        {
            ...
            {
                From = _fromEmail,
                Subject = $"Welcome to {hotelName} — {referenceNumber}"
            };
            message.To.Add(toEmail);
            message.HtmlBody = BuildCheckInHtml(guestName, referenceNumber, hotelName, checkOut);
        ...
        private static string BuildCancellationHtml(
            string guestName, string referenceNumber, string hotelName,
            DateTime checkIn, DateTime checkOut, decimal cancellationFee) => $"""
            <!DOCTYPE html>
            <html lang="en">
            <head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
            <body style="font-family:Arial,sans-serif;background:#f4f4f4;margin:0;padding:0;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:30px 0;">
                <tr><td align="center">
                  <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;">
                    ...
                    <tr><td style="padding:30px;">
                      <p style="color:#2d3748;font-size:16px;">Dear {guestName},</p>
                      <p style="color:#4a5568;">Your booking has been cancelled. Here are the details:</p>
                      <table width="100%" cellpadding="12" cellspacing="0" style="background:#f7fafc;border-radius:6px;margin:20px 0;">
                        <tr><td style="color:#718096;font-size:14px;">Reference</td>
                            <td style="color:#1a202c;font-weight:bold;text-align:right;">{referenceNumber}</td></tr>
                        ...
                        <tr><td style="color:#718096;font-size:14px;">Cancellation Fee</td>
                            <td style="color:#c53030;font-weight:bold;font-size:18px;text-align:right;">£{cancellationFee:F2}</td></tr>
                      </table>
                      {(cancellationFee == 0
                          ? "<p style=\"color:#38a169;\">No cancellation fee applies — full refund will be processed.</p>"
                          : "<p style=\"color:#718096;font-size:13px;\">The cancellation fee will be deducted from your refund where applicable.</p>")}
                    </td></tr>
                    <tr><td style="background:#1a1a2e;padding:20px;text-align:center;">
                      <p style="color:#a0aec0;font-size:12px;margin:0;">© {DateTime.UtcNow.Year} HMS Hotel Management System</p>
                    </td></tr>
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:30px 0;">
                <tr><td align="center">
                  <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;">
                    ...
                    <tr><td style="padding:30px;">
                      <p style="color:#2d3748;font-size:16px;">Dear {guestName},</p>
                      <p style="color:#4a5568;">You have successfully checked in. Here is a reminder of your booking details:</p>
                      <table width="100%" cellpadding="12" cellspacing="0" style="background:#f7fafc;border-radius:6px;margin:20px 0;">
                        <tr><td style="color:#718096;font-size:14px;">Reference</td>
                            <td style="color:#1a202c;font-weight:bold;text-align:right;">{referenceNumber}</td></tr>
                      ...
                      <p style="color:#4a5568;">If you need any assistance during your stay, please do not hesitate to contact the front desk.</p>
                    </td></tr>
                    <tr><td style="background:#1a1a2e;padding:20px;text-align:center;">
                      <p style="color:#a0aec0;font-size:12px;margin:0;">© {DateTime.UtcNow.Year} HMS Hotel Management System</p>
                    </td></tr>
